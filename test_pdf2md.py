...
class TestVlmFallback(unittest.TestCase):
    ...
    def test_render_page_as_image_returns_pil_image(self):
        ...
        with patch('pdf2md.fitz') as mock_fitz, \
             patch('pdf2md.Image') as mock_pil_image_module, \
             patch('pdf2md.io') as mock_io:
    ...
    def test_thin_page_triggers_vlm(self):
        ...
        with patch('pdf2md.extract_pages', return_value=thin_pages), \
             patch('pdf2md.extract_page_via_vlm', return_value='VLM extracted text') as mock_vlm:
    ...
    def test_normal_page_skips_vlm(self):
        ...
        with patch('pdf2md.extract_pages', return_value=normal_pages), \
             patch('pdf2md.extract_page_via_vlm') as mock_vlm:
            results = extract_pages_hybrid(
                '/fake/doc.pdf',
                ocr=True,
                force_ocr=False,
    ...
    def test_force_ocr_uses_vlm_on_all_pages(self):
        ...
        with patch('pdf2md.extract_pages', return_value=all_pages), \
             patch('pdf2md.extract_page_via_vlm', return_value='VLM result') as mock_vlm:
            results = extract_pages_hybrid(
                '/fake/doc.pdf',
                ocr=False,
                force_ocr=True,
    ...
    def test_no_ocr_flag_skips_vlm_entirely(self):
        ...
        with patch('pdf2md.extract_pages', return_value=pages), \
             patch('pdf2md.extract_page_via_vlm') as mock_vlm:
    ...
    def test_extract_page_via_vlm_cleans_up_temp_file(self):
        ...
        with patch('pdf2md.render_page_as_image', return_value=fake_image), \
             patch('pdf2md.tempfile') as mock_tempfile, \
             patch('pdf2md.os.unlink') as mock_unlink, \
             patch('pdf2md.apply_chat_template', return_value='formatted_prompt'), \
             patch('pdf2md.generate', return_value='Extracted markdown text') as mock_gen:
